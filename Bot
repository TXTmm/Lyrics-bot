...
local function botDrawLots()
	...
	if drawLots == 10 then 
		game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("🤖 Fun fact: Did you know that this robot was made by an unemployed human.", "All")		
	...
	elseif drawLots == 7 then 
		...
		game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("🤖 Hello! My name is "..game.Players.LocalPlayer.Name..", and I'm a robot! I have a library with millions of songs! Use the command ;lyrics [song name] to see if I can sing your favorite songs.", "All")
	end
end

-- Send first tooltip 
game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("🤖 Hello! My name is "..game.Players.LocalPlayer.Name..", and I'm a robot! I have a library with millions of songs! Use the command ;lyrics [song name] to see if I can sing your requested songs.", "All")
		...
		if not string.match(sentMessage,"#") then -- check if there's hashtags
			if sentMessage ~= ";lyrics" and sentMessage ~= ";lyrics " then -- Make sure it's not some retard just sending ;lyrics 
				...
				if songLyrics ~= false then  
					...
					saySomething("🤖 Success, "..songRequester.."! The song was found in my API's libraries. The requested song will last for about "..tostring(shared.songMinutesCount).." minutes. Please keep in mind that I cannot take song requests while singing.") -- announce the bullshit 
